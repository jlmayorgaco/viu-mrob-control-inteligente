...
El sistema R2ET tiene no linealidad leve (saturación del actuador), perturbaciones de amplitud conocida pero duración variable, y la necesidad de coordinar dos variables de proceso acopladas a través del reparto del robot. Ninguno de estos rasgos impide usar un PID, pero sí limita su rendimiento: el PID no coordina esteras de forma nativa ni adapta sus parámetros ante la variación de peso. Por eso se evalúan tres alternativas inteligentes junto con el PID como referencia.
...
Cada método destaca en un criterio distinto. La lógica difusa es la más recomendable para un despliegue auditable: sus 35~reglas son legibles y modificables por el personal de planta sin herramientas de aprendizaje automático. El PID retuneado da el menor IAE en la coordinación de dos esteras; el Q-Learning es el más eficiente en energía y el más robusto; y la red profunda ofrece el mejor compromiso en el lazo servo (IAE $2\times$ menor que el PID en Fase~1, con el mando más suave y sin saturar) y, en la coordinación, el mayor margen de seguridad y productividad.
...
Intuición y pasos. La dificultad es que cada acción $u(k)$ afecta no solo al error inmediato, sino a toda la trayectoria posterior de $y$; por eso no basta con derivar muestra a muestra. El co-estado $\lambda(k)$ resume \emph{cuánto} contribuye el estado en el instante $k$ al error total acumulado hasta el final del episodio. El cálculo tiene tres pasos:
\begin{enumerate}
