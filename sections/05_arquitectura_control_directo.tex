\chapter{4.2.c Arquitectura de control directo inteligente}
\label{ch:arquitectura}

Nota sobre herramienta de simulación. La arquitectura se documenta en dos formatos complementarios: diagramas vectoriales (TikZ) para la explicación funcional y modelos Simulink del lazo de control y del supervisor (las capturas correspondientes se incluyen en cada apartado). La simulación cuantitativa (entrenamiento, métricas y figuras de resultados) corre en Python~3.13 con NumPy, con reproducibilidad exacta (semilla fija SEED=8, sin dependencia de versión de toolbox). La planta empleada en ambos entornos es la misma discretización ZOH de la función de transferencia $G_p(s)$ asignada en la guía (Capítulo~\ref{ch:contexto}). El código fuente completo figura en el Apéndice~A.

\section{Fase 1: Servoregulador}

La Fase~1 valida cada controlador sobre la planta de primer orden discreta del Grupo~1 (ecuación~\ref{eq:planta_servo}), actuando como servoregulador. La señal de referencia es un escalón al nivel de operación $r=0.6$ activado en $k=10$; la perturbación de carga ($d=1$, cambio de peso en las piezas) actúa entre las muestras $k=85$ y $k=110$.
...
\simulinkfigure{figures/simulink/sl_pid_servo.png}{0.85}{Modelo Simulink del lazo servo digital (Fase~1): referencia, PID discreto, saturación del actuador y planta discreta (ZOH de $G_p(s)$) con perturbación de carga.}
...
Planta & Dinámica de la estera & $y(k{+}1) = 0.92\,y + 0.08\,u - 0.024\,d$; $a=0.92$, $b=0.08$, $b_d=0.024$.\\
...
\end{tikzpicture}}
\caption{Flujo de cálculo del controlador difuso Mamdani: fuzzificación → inferencia AND-mín con 35 reglas → defuzzificación por promedio ponderado de los consecuentes.}
\label{fig:fuzzy_flow}
\end{figure}

\simulinkfigure{figures/simulink/sl_control_directo.png}{0.9}{Modelo Simulink de la arquitectura de control digital directo inteligente (4.2.c): referencia → error → controlador inteligente directo (fuzzificación, inferencia AND-mín, defuzzificación WA) → saturación → planta. El bloque del controlador es intercambiable (Fuzzy/DL/QL).}
...
\begin{tikzpicture}[>=Latex, every node/.style={font=\scriptsize}]
  ...
  \node[inp] (de1) at (0, 1.35)  {$\Delta n_1$};
  \node[inp] (de2) at (0, 0.45)  {$\Delta n_2$};
...
\end{tikzpicture}}
\caption{Arquitectura de despliegue del Q-Learning servo (Fase~1): codificación del estado → consulta greedy de la tabla Q → acción discreta de velocidad.}
\label{fig:ql_arch}
\end{figure}
...
\end{tikzpicture}}
\caption{Arquitectura de control aware Fase~2: controlador primario + supervisor de capacidad (capa de seguridad) → esteras R2ET. La perturbación $d$ y el caudal $\lambda$ del robot entran al modelo de las esteras.}
\label{fig:control_aware}
\end{figure}

\simulinkfigure{figures/simulink/sl_r2et_aware.png}{0.9}{Modelo Simulink de la coordinación R2ET (Fase~2): controlador genera $u_1,u_2,r_1$; el supervisor de capacidad actúa como barrera de seguridad sobre las dos esteras (capacidad 3~piezas).}
