\chapter{Arquitectura de control directo inteligente}
\label{ch:arquitectura}

Correspondencia con la guía: apartado 4.2.c. La arquitectura se documenta en dos formatos complementarios: diagramas vectoriales (TikZ) para la explicación funcional y modelos Simulink del lazo de control y del supervisor (las capturas correspondientes se incluyen en cada apartado). La simulación cuantitativa (entrenamiento, métricas y figuras de resultados) corre en Python~3.13 con NumPy, con semilla fija SEED=8 y sin dependencia de versiones de toolbox. La planta servo empleada en los ensayos cuantitativos es el modelo reducido de primer orden obtenido de la dinámica dominante de $G_p(s)$ y discretizado por ZOH (Capítulo~\ref{ch:contexto}); la Fase~2 usa el balance de masa R2ET sin recorte superior de capacidad. El Apéndice~A muestra extractos representativos y los scripts completos se entregan junto con el informe.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Planta .png}
    \caption{Diagrama de bloques para la representación discreta del comportamiento de la planta.}
\end{figure}


\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Diagrama de bloques PID.png}
    \caption{Modelo Simulink del lazo servo digital (Fase~1): referencia, PID discreto, saturación del actuador y modelo reducido discretizado por ZOH a partir de $G_p(s)$, con perturbación de carga.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Salida controlador PID de planta discreta.png}
    \caption{Salida del sistema: lazo cerrado con controlador PID discreto, saturación del actuador y modelo reducido discretizado por ZOH, con perturbación de carga.}
\end{figure}
...
Controlador & Calcula el mando $u^*$ & PID: anti-windup; Fuzzy: reglas Mamdani; DL: inferencia red; QL: tabla Q greedy.\\
...
\subsubsection*{Controlador difuso Mamdani}
...
\end{tikzpicture}}
\caption{Flujo de cálculo del controlador difuso Mamdani: fuzzificación $\rightarrow$ inferencia AND-mín con 35 reglas $\rightarrow$ defuzzificación por promedio ponderado de los consecuentes.}
\label{fig:fuzzy_flow}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Control difuso diagrama bloques.png}
    \caption{Modelo Simulink de la arquitectura de control digital directo inteligente (4.2.c): referencia $\rightarrow$ error $\rightarrow$ controlador inteligente directo (fuzzificación, inferencia AND-mín, defuzzificación WA) $\rightarrow$ saturación $\rightarrow$ planta. El bloque del controlador es intercambiable (Fuzzy/DL/QL).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Controlador inteligente directo.png}
    \caption{Subsistema del controlador inteligente directo basado en las entradas $e$, $\Delta e$ y $d$, con salida de control $u^*(k)$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Controlador difuso mamdani.png}
    \caption{Respuesta del sistema ante entrada escalón: comparación entre la referencia y la salida (y(k)) de la planta discreta.}
\end{figure}

Validación Simulink--Python. Las capturas Simulink de esta sección documentan la estructura de bloques y una demo de lazo cerrado; los indicadores cuantitativos del Capítulo~\ref{ch:pruebas} proceden del banco Python, que usa la sintonía final y el mismo modelo discreto de la ecuación~\ref{eq:planta_servo}. Por eso los scopes de Simulink pueden mostrar un régimen permanente visualmente cercano a $0.52$--$0.54$ frente a $r=0.6$: corresponden a una sintonía de demostración previa, no al banco final usado para las tablas. En Python, con la sintonía definitiva, el error estacionario de las últimas 20 muestras queda por debajo del criterio RF-07 para los cuatro métodos (Sección~\ref{ch:pruebas}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Sistema_completo_R2ET.png}
    \caption{Modelo Simulink de la coordinación R2ET (Fase~2): controlador genera $u_1,u_2,r_1$; el supervisor de capacidad actúa como barrera de seguridad sobre las dos esteras (capacidad 3~piezas).}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Controlador_primario.png}
    \caption{Subsistema del controlador primario R2ET, encargado de generar las señales propuestas $u_1^*$, $u_2^*$ y $r_1^*$ a partir de la referencia de ocupación y los estados $n_1$ y $n_2$.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Supervisor_capacidad_R2ET.png}
    \caption{Subsistema supervisor de capacidad, implementado como barrera de seguridad para corregir las señales de control y evitar el desbordamiento de las esteras.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/Esferas_R2ET.png}
    \caption{Subsistema de esteras R2ET, donde se modela la dinámica discreta de ocupación $n_1(k)$ y $n_2(k)$, considerando caudal efectivo, reparto del robot, descarga y perturbación.}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=5in]{images/salida sistema R2ET.png}
    \caption{Respuesta de ocupación de las esteras R2ET ante perturbación: comparación de $n_1(k)$, $n_2(k)$, referencia de 1.5 piezas y capacidad máxima de 3 piezas. En esta demo el controlador primario es proporcional puro ($K_u=0.5$), por lo que aparece el offset estacionario esperado de un regulador sin acción integral; el PID de ocupación supervisado del Capítulo~\ref{ch:supervisor} elimina ese sesgo y converge a $n^*=1.5$.}
\end{figure}
