...
La dificultad no está en hacer que el robot alterne, sino en decidir \emph{cuánto} alimentar a cada estera y \emph{a qué velocidad} descargarlas. Tres situaciones lo complican:

Saturación. Si el robot sigue depositando piezas en una estera ya casi llena, el margen se agota en pocas muestras. El control debe anticipar la saturación y redirigir el flujo.

Vaciado. Si la velocidad de descarga es demasiado alta respecto al flujo de entrada, la estera queda vacía y la línea de salida se interrumpe. El nivel objetivo de~1.5~piezas ($50\%$ de la capacidad) centra la operación dejando margen de seguridad en ambos sentidos (frente a saturación y frente a vaciado).

Cambio de peso. Piezas más pesadas se mueven con más fricción sobre la estera: la eficiencia de descarga cae hasta un $35\%$ en E1 y un $17.5\%$ en E2 durante el episodio de perturbación. El controlador debe detectar esta caída y compensarla (acelerando la estera, reduciendo la admisión de nuevas piezas o redirigiendo el robot hacia E2) sin esperar a que la estera llegue al umbral de alarma.
...
\begin{itemize}
  ...
  \item Factor de amortiguamiento: $\zeta = \dfrac{0.785}{2\,\omega_n} = \dfrac{0.785}{1.224} = 0.641$ \quad (subamortiguado bien amortiguado)
...
El período de muestreo de la planta servo se fija por el criterio de doce muestras por constante de tiempo dominante: $T_s = \tau_d/12 = 0.2125$~s. Conviene distinguir dos escalas temporales: este $T_s$ es el de la dinámica servo local de la Fase~1, mientras que la coordinación de la celda R2ET (Fase~2) y las capas de Industria~4.0 operan a un ciclo nominal mayor, de $1$~s. La discretización por retenedor de orden cero (ZOH) de un primer orden da el polo discreto $a = e^{-T_s/\tau_d}$, con $b = K_f(1-a)$ y $b_d = K_d(1-a)$:
...
En la Fase~2 se simula el balance de masa de las dos esteras:

\begin{equation}
n_i(k+1) = \operatorname{sat}_{[0,\,3]}\!\left[\,n_i(k)
  + \underbrace{\lambda(k)\,a(k)\,r_i}_{\text{entrada}}
  - \underbrace{\eta\,u_i(k)\,\bigl[1-\ell_i(k)\bigr]}_{\text{salida}}\,\right]
\label{eq:r2et_aware}
\end{equation}
...
Umbral de riesgo & 2.4 piezas & A partir de aquí el supervisor refuerza el control.\\
