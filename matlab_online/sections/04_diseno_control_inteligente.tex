\chapter[Diseño del control inteligente]{Diseño del sistema de control inteligente\textsuperscript{\normalfont *}}
\label{ch:diseno}
\begingroup\renewcommand\thefootnote{*}\renewcommand\theHfootnote{guide.diseno}\footnotetext{Este capítulo responde a los apartados 4.2.a y 4.2.b de la guía de la actividad.}\endgroup

\section{Selección y justificación de la estrategia de control}

El sistema R2ET presenta tres características que dificultan el control clásico: una no linealidad leve (la saturación del actuador), perturbaciones de amplitud conocida pero duración variable, y dos variables de proceso acopladas por el reparto del robot. Estas características no impiden emplear un PID, pero sí acotan las prestaciones que puede alcanzar, porque un PID no coordina esteras por sí mismo ni reajusta sus parámetros cuando cambia el peso. De ahí que, junto al PID de referencia, se evalúen tres alternativas inteligentes.
...
PID & Clásico: referencia & Elimina el error estacionario por acción integral. Es un método bien conocido, fácil de auditar y de mantener. Limitación: un PID escalar por estera no coordina el reparto del robot ni adapta parámetros ante la perturbación de peso.\\[4pt]
Fuzzy & Reglas lingüísticas & Las reglas lingüísticas son auditables sin conocimiento de sistemas de control avanzados. No necesita datos históricos de la planta. Incorpora directamente el conocimiento de un experto sobre cuándo acelerar, frenar o redirigir el robot \cite{ross2010fuzzy,kasabov1996foundations}.\\[4pt]
Deep Learning & Optimización directa sobre planta & Entrena la política \emph{offline} sobre el gemelo digital, minimizando el IAE mediante retropropagación a través de la simulación discreta. La inferencia en despliegue es de coste computacional fijo por ciclo \cite{kasabov1996foundations}.\\[4pt]
...
Para la propuesta de control inteligente se selecciona el controlador difuso integrado con el supervisor de capacidad. La decisión prioriza una primera implantación industrial: sus 35~reglas son auditables, no requiere datos históricos y permite al personal de planta ajustar la lógica sin herramientas de aprendizaje automático. El PID queda como línea base y respaldo industrial; Deep Learning y Q-Learning se incluyen como comparación avanzada y como alternativas para validación en gemelo digital o modo sombra.

La comparación cuantitativa acota el papel de cada alternativa. El PID resintonizado marca el menor IAE de la coordinación de dos esteras. El Q-Learning obtiene el menor consumo energético y la mejor robustez en el escenario severo. La red profunda combina IAE bajo en el lazo servo con mayor margen de seguridad y productividad en Fase~2.

En las tablas y títulos se conserva la etiqueta \emph{aware} para las configuraciones que usan el estado completo de la celda (ocupación de ambas esteras, reparto del robot, admisión y perturbación). En el texto equivale a coordinación consciente del estado de la celda.

\section{Variables, universos de discurso y señales de retroalimentación}
...
$e = r - y$ (servo) & $[-1.5,\;1.5]$ & NL, NM, NS, ZE, PS, PM, PL & Error de seguimiento en Fase~1\\
...
$u_i$ & $[0.05,\;0.98]$\newline(norm.) & 7 niveles (FAM por nivel de error/derivada) & Velocidad de descarga de estera $i$.\footnote{El recorte superior a 0.98 es propio de la FAM del controlador difuso; la restricción física del actuador (Tabla~\ref{tab:var_proceso}) sigue siendo $[0.05,\,1.0]$ y los demás métodos pueden alcanzar $u=1.0$.}\\
...
La integral $\sigma(k)$ usa anti-windup condicional: solo se actualiza cuando el actuador no está saturado en la misma dirección que el error:
\[
...
\section{Controlador difuso (Mamdani-singleton / Sugeno de orden cero)}

El controlador combina antecedentes lingüísticos con inferencia AND-mínimo, al estilo Mamdani, y consecuentes singleton con defuzzificación por promedio ponderado; esta configuración equivale formalmente a un sistema Sugeno de orden cero \cite[cap.~5]{passino2005biomimicry}, y así se implementa en MATLAB mediante \texttt{sugfis} (Listing~\ref{lst:fis_control_directo}).

\subsection*{Base de conocimiento}

La base de conocimiento del controlador difuso tiene dos componentes \cite[cap.~5]{passino2005biomimicry}:
\begin{itemize}
  \item Base de datos: las variables lingüísticas, sus universos de discurso y las funciones de membresía de la Tabla~\ref{tab:variables_ling} y la estrategia de fuzzificación siguiente.
...
\artifactfigure{figures/plots/fig_fuzzy_mf.png}{0.96}{Funciones de membresía: error $e$ (7~conjuntos triangulares/trapezoidales), derivada $\Delta e$ (5~conjuntos), perturbación $d$ (2~conjuntos) y velocidad de salida por nivel de error para Fase~1 y Fase~2.}

Se emplean 7 conjuntos para el error (resolución densa cerca de cero) y 5 para la derivada, lo que da una base de 35 reglas (frente a las 15 de una partición $5\times3$) y un control más fino cerca del punto de operación. El diseño de las funciones de membresía, la inferencia y la defuzzificación sigue el procedimiento estándar de control basado en reglas \cite[cap.~5]{passino2005biomimicry}, \cite{ross2010fuzzy}.
...
Regla de perturbación: SI $d=\text{CON}$ ENTONCES $u=u_{base}+0.30$, con intensidad $\mu_{CON}(d)$, activa solo cuando $d>0.35$ (umbral de la MF trapecial). Se combina en paralelo con las 35 reglas en el promedio ponderado, reforzando la descarga ante el atasco sin interferir con el régimen nominal.
...
La red es un perceptrón multicapa que actúa como aproximador no lineal de la política de control \cite[cap.~4]{passino2005biomimicry}. Ambas redes se entrenan \emph{offline} sobre el gemelo digital de la planta: la pérdida combina IAE y penalización de suavidad, y su gradiente se obtiene propagando el error a través de la simulación discreta. En las ecuaciones siguientes se presenta el método adjunto discreto usado para calcular ese gradiente de forma explícita.
...
Este gradiente se retropropaga luego a través de la red (retropropagación estándar) y los pesos se actualizan con Adam con planificación coseno de la tasa de aprendizaje (\emph{cosine annealing}).

Interpretación física. Cada acción $u(k)$ no afecta solo al error de ese instante: arrastra toda la trayectoria posterior de $y$. Derivar muestra a muestra, por tanto, no basta. El co-estado $\lambda(k)$ resume cuánto pesa el estado del instante $k$ en el error acumulado hasta el final del episodio. El cálculo va en tres pasos:
\begin{enumerate}
  ...
  \item Pasada hacia atrás: se parte de $\lambda(T)$ (el error en el instante final) y se recorre el tiempo en sentido inverso; en cada paso se suma el error local más la contribución que llega del futuro, $A\,\lambda(k+1)$. Es la misma idea de la retropropagación a través del tiempo (\emph{backpropagation through time}).
  \item Gradiente: $\partial L/\partial u(k)=B\,\lambda(k+1)$ indica cómo mover cada acción para reducir el IAE; ese valor se inyecta como error en la capa de salida de la red y se retropropaga con retropropagación estándar.
\end{enumerate}
En la práctica equivale a derivar la simulación completa respecto a las acciones de la red. La señal de aprendizaje no proviene de una política supervisora externa, sino del error acumulado que la propia política produce sobre la planta simulada.

\subsection*{Fase~1: Servorregulador}

Estado de entrada: $[e,\;\Delta e,\;\sigma,\;d]$ con $\sigma(k)=\sum_{j=0}^{k}e(j)$ (integral acumulada). La integral distingue error estacionario acumulado de estado puro; sin ella la red no puede eliminar el offset. Arquitectura: $4\!\to\!64\!\to\!32\!\to\!16\!\to\!8\!\to\!1$ (sigmoide en salida). Entrenamiento: 800~épocas $\times$ $B=32$ escenarios aleatorios ($y_0\sim U(0,0.4)$, perturbación aleatoria). Pérdida: $L=\text{IAE}+\lambda_u\sum|\Delta u|^2$, con $\lambda_u=0.01$ para penalizar cambios bruscos del mando.

\subsection*{Fase~2: Coordinación aware}

El mismo principio adjunto se extiende a la planta R2ET de dos esteras. El estado tiene 8 entradas:
\[
x(k)=\big[e_1,\,e_2,\,\Delta e_1,\,\Delta e_2,\,\sigma_1,\,\sigma_2,\,b,\,d\big].
\]
La arquitectura es más profunda que en el servo, $8\!\to\!96\!\to\!64\!\to\!32\!\to\!16\!\to\!8\!\to\!3$ (tres salidas sigmoide: $u_1$, $u_2$, $r_1$), por la mayor complejidad del problema de dos esteras. El co-estado se calcula para cada estera por separado ($\lambda_1$, $\lambda_2$) y el gradiente respecto a $r_1$ incorpora el efecto del reparto sobre la alimentación de cada estera. Entrenamiento: 1200~épocas $\times$ $B=16$ escenarios ($n_0\sim U(0.4,2.6)$, atasco aleatorio con probabilidad $0.55$, demanda $\pm15\%$).

\artifactfigure{figures/plots/fig_dl_training.png}{0.90}{Curvas de entrenamiento. Izq.: Fase~1 servo: IAE normalizado por optimización directa (método adjunto, 800~épocas $\times$ 32~escenarios). Der.: Fase~2 aware: IAE normalizado por método adjunto de dos esteras (1200~épocas $\times$ 16~escenarios). Escala logarítmica.}

\input{artifacts/tables/mlp_training_summary.tex}
...
\toprule
Métrica & Servo ($4{\to}1$) & Aware ($8{\to}3$) \\ \midrule
$L_{train}$ (IAE normaliz.) & 0.00406 & 0.199 \\
$L_{val}$ (IAE normaliz.)   & 0.00432 & 0.175 \\
Brecha de generalización $\Delta_{gen}$ & $\mathbf{6.4\%}$ & $-12.1\%$ \\
...
Optimizador & Adam, coseno LR & Adam, coseno LR \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}
  \item Servo: $s=(e_{bin},\,\Delta e_{bin},\,d_{bin})$ $\rightarrow$ $10\times5\times2=100$ estados; 8~acciones de velocidad discreta.
  \item Aware: $s=(n_{1,bin},\,n_{2,bin},\,d_{bin})$ $\rightarrow$ $9\times9\times2=162$ estados; 11~combinaciones de $(u,\,r_1,\,a)$.
\end{itemize}
...
\]
Los pesos altos de \emph{viol.} y \emph{alarma} (30 y 8) hacen que el agente priorice no superar la capacidad; el resto afina el seguimiento y el equilibrio.
...
\input{artifacts/tables/ql_training_summary.tex}
