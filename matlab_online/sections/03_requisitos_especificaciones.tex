\chapter[Requisitos y especificaciones]{Análisis de requisitos y especificaciones técnicas\textsuperscript{\normalfont *}}
\label{ch:requisitos}
\begingroup\renewcommand\thefootnote{*}\renewcommand\theHfootnote{guide.requisitos}\footnotetext{Este capítulo responde al apartado 4.1 de la guía de la actividad.}\endgroup

Los requisitos se derivan de las restricciones físicas de la celda, los criterios de seguridad operativa y los objetivos de producción. El sistema de control prioriza en este orden: seguridad operativa (cero desbordamientos), estabilidad de la ocupación en torno al nivel objetivo, y eficiencia energética con máxima productividad.
...
\label{tab:req_func}
\begin{tabular}{@{}P{0.11\linewidth}P{0.23\linewidth}P{0.55\linewidth}@{}}
\toprule
ID & Requisito & Descripción \\ \midrule
...
RF-09 & Servorregulación local & Regular $y(k)$ ante escalón y perturbación en la planta de Fase~1.\\
...
\toprule
ID & Requisito & Descripción \\ \midrule
RNF-01 & Precisión & IAE (Fase~2) $<60$ en 160~muestras; error estacionario $<2\%$ para la estrategia seleccionada.\\
...
RNF-10 & Trazabilidad & Registro temporal de todas las variables para análisis posterior de eventos.\\
\bottomrule
\end{tabular}
\end{table}

\section{Especificaciones técnicas del sistema de control}
...
\begin{tabular}{@{}P{0.18\linewidth}P{0.13\linewidth}P{0.13\linewidth}P{0.44\linewidth}@{}}
\toprule
Variable & Rango & Unidad & Descripción \\ \midrule
...
\label{tab:params_op}
\begin{tabular}{@{}P{0.30\linewidth}P{0.15\linewidth}P{0.43\linewidth}@{}}
\toprule
...
Período de muestreo & 1~s & Ciclo de control de la celda R2ET; la planta servo de Fase~1 usa $T_s \approx 0.2125$~s (Capítulo~\ref{ch:contexto}).\\
\bottomrule
\end{tabular}
\end{table}
...
\toprule
Indicador & Objetivo & Criterio de aceptación \\ \midrule
